...
class IntelA (joueur.Joueur):
    ...
    def lancementDeIA(self, interface) -> int:
        ...
        interface.get_de_jeu().set_faceDe(randint(1,6))

        if interface.get_de_jeu().get_faceDe() == 1:
            # Affiche le de sur la face 1
            de_face1 = image.De.FACE1.value
            interface.affichageAction(de_face1)
            
    
        elif interface.get_de_jeu().get_faceDe() == 2:
            # Affiche le de sur la face 2
            de_face2 = image.De.FACE2.value
            interface.affichageAction(de_face2)
            
    
        elif interface.get_de_jeu().get_faceDe() == 3:
            # Affiche le de sur la face 3
            de_face3 = image.De.FACE3.value
            interface.affichageAction(de_face3)
            
    
        elif interface.get_de_jeu().get_faceDe() == 4:
            # Affiche le de sur la face 4
            de_face4 = image.De.FACE4.value
            interface.affichageAction(de_face4)
            
    
        elif interface.get_de_jeu().get_faceDe() == 5:
            # Affiche le de sur la face 5
            de_face5 = image.De.FACE5.value
            interface.affichageAction(de_face5)
            
    
        elif interface.get_de_jeu().get_faceDe() == 6:
            # Affiche le de sur la face 6
            de_face6 = image.De.FACE6.value
            interface.affichageAction(de_face6)
            
    
        return interface.get_de_jeu().get_faceDe()
    ...
    def choixDirectionIA(self, interface):
        ...
        for i in interface.getPlateau().getCasesDecouvertes():
            distance = math.sqrt((case_depart[0] - i[0])**2 + (case_depart[1] - i[1])**2)
            if (distance == interface.get_de_jeu().get_faceDe()):
                if (interface.getPlateau().get_cases(i[0],i[1]) == interface.getCouleur().getRose() and distance < case_recompense["case_rose"]["distance"]):
                    case_recompense["case_rose"]["coord"] = ([i[0],i[1]])
                    case_recompense["case_rose"]["distance"] = distance
                elif (interface.getPlateau().get_cases(i[0],i[1]) == interface.getCouleur().get_Rouge() and distance < case_recompense["case_rouge"]["distance"]):
                    case_recompense["case_rouge"]["coord"] = ([i[0],i[1]])
                    case_recompense["case_rouge"]["distance"] = distance
                elif (interface.getPlateau().get_cases(i[0],i[1]) == interface.getCouleur().get_Violet() and distance < case_recompense["case_violet"]["distance"]):
                    case_recompense["case_violet"]["coord"] = ([i[0],i[1]])
                    case_recompense["case_violet"]["distance"] = distance
                elif (interface.getPlateau().get_cases(i[0],i[1]) == interface.getCouleur().get_Bleu() and distance < case_recompense["case_bleu"]["distance"]):
                    case_recompense["case_bleu"]["coord"] = ([i[0],i[1]])
                    case_recompense["case_bleu"]["distance"] = distance
                elif (interface.getPlateau().get_cases(i[0],i[1]) == interface.getCouleur().getNoir() and distance < case_recompense["case_noir"]["distance"]):
                    case_recompense["case_noir"]["coord"] = ([i[0],i[1]])
                    case_recompense["case_noir"]["distance"] = distance
                elif (interface.getPlateau().get_cases(i[0],i[1]) == interface.getCouleur().get_Orange() and distance < case_recompense["case_orange"]["distance"]):
                    case_recompense["case_orange"]["coord"] = ([i[0],i[1]])
                    case_recompense["case_orange"]["distance"] = distance
                elif (interface.getPlateau().get_cases(i[0],i[1]) == interface.getCouleur().getBlanc() and distance < case_recompense["case_blanc"]["distance"]):
                    case_recompense["case_blanc"]["coord"] = ([i[0],i[1]])
                    case_recompense["case_blanc"]["distance"] = distance
                elif (interface.getPlateau().get_cases(i[0],i[1]) == interface.getCouleur().get_Indigo() and distance < case_recompense["case_indigo"]["distance"]):
                    case_recompense["case_indigo"]["coord"] = ([i[0],i[1]])
                    case_recompense["case_indigo"]["distance"] = distance
                elif (interface.getPlateau().get_cases(i[0],i[1]) == interface.getCouleur().get_Vert() and distance < case_recompense["case_vert"]["distance"]):
                    case_recompense["case_vert"]["coord"] = ([i[0],i[1]])
                    case_recompense["case_vert"]["distance"] = distance
                elif (interface.getPlateau().get_cases(i[0],i[1]) == interface.getCouleur().get_Jaune() and distance < case_recompense["case_jaune"]["distance"]):
                    case_recompense["case_jaune"]["coord"] = ([i[0],i[1]])
                    case_recompense["case_jaune"]["distance"] = distance
                elif (interface.getPlateau().get_cases(i[0],i[1]) == interface.getCouleur().getGris() and distance < case_recompense["case_gris"]["distance"]):
                    case_recompense["case_gris"]["coord"] = ([i[0],i[1]])
                    case_recompense["case_gris"]["distance"] = distance
                elif (interface.getPlateau().get_cases(i[0],i[1]) == interface.getCouleur().get_Beige() and distance < case_recompense["case_beige"]["distance"]):
        ...
        if (interface.getPlateau().get_cases(self.getPlateauX(),self.getPlateauY()) == interface.getCouleur().get_Jaune() and "case_jaune" in liste_case):
        ...
        while interface.get_de_jeu().get_faceDe() != 0:
            if self.getPlateauX() < case_arrivee[0]:
                # La touche fleche vers le haut a ete enfoncee
                self.haut(47)
                interface.getPlateau().setCasesDecouvertes(interface.getPlateau().getCasesDecouvertes() + [[self.getPlateauX(),self.getPlateauY()]])
                
                interface.affichage_image_plateau(self)
                interface.get_de_jeu().desincrement_faceDe(1)
    ...
    def choixActionCombatIA(self, interface):
        if self.adv_a_attaquer(interface.getListeJoueurs()) == True:
            self.adv_a_attaquer(interface.getListeJoueurs())
            ordre_adv = self.adv_nom_attaquer(interface.getListeJoueurs())
            if ordre_adv != -1:
                self.combat_joueurs(interface, ordre_adv)
        else:
            for k in interface.getListeJoueurs():
                if (interface.getPlateau().getCasesDecouvertes()[k.getPlateauX()][k.getPlateauY()] != "case_rouge"):
